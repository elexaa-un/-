...
var objectOpacity = 0.5; // 初始设为 0.5 (半透明)
...
window.onload = function () {
	...
	// 开启混合
	gl.enable(gl.BLEND);
	// 设置混合因子：源因子(当前绘制物体的Alpha), 目标因子(背景颜色的1-Alpha)
...
function render() {
	...
	// ==============================================
	// 3. 最后绘制半透明物体：立方体和地板 (Cube & Floor)
	// ==============================================
	gl.useProgram(program);
	// 传递透明度变量
	gl.uniform1f(gl.getUniformLocation(program, "u_opacity"), objectOpacity);
	...
	gl.uniformMatrix4fv(gl.getUniformLocation(program, "u_ModelMatrix"), false, flatten(ModelMatrix)); // 确保传了 ModelMatrix
	gl.drawArrays(gl.TRIANGLES, 0, cubenumPoints);
...
window.onkeydown = function (e) {
	...
	switch (code) {
		...
		case 49: // 按键 '1' : 减少透明度 (更透明)
			objectOpacity -= 0.1;
			if (objectOpacity < 0.0) objectOpacity = 0.0;
			console.log("Opacity:", objectOpacity);
			break;
		case 50: // 按键 '2' : 增加透明度 (更不透明)
			objectOpacity += 0.1;
			if (objectOpacity > 1.0) objectOpacity = 1.0;
			console.log("Opacity:", objectOpacity);
			break;
	}
